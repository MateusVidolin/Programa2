...
exports.getAll = (req, res, next) => {
  ...
  const filePath = req.query.msgFilePath;
  ...
  Impressao.findAll({
    ...
    impressaos.sort((a, b) => a.professor.nome.localeCompare(b.professor.nome));
    const nomesMateriasAtribuidas = impressaos.map((impressao) => {
      return {
        id: impressao.id,
        disciplinas: [
          impressao.IdDisciplina1
            ? impressao.IdDisciplina1.nomeDisciplina
            : null,
          impressao.IdDisciplina2
            ? impressao.IdDisciplina2.nomeDisciplina
            : null,
          impressao.IdDisciplina3
            ? impressao.IdDisciplina3.nomeDisciplina
            : null,
          impressao.IdDisciplina4
            ? impressao.IdDisciplina4.nomeDisciplina
            : null,
          impressao.IdDisciplina5
            ? impressao.IdDisciplina5.nomeDisciplina
            : null,
          impressao.IdDisciplina6
            ? impressao.IdDisciplina6.nomeDisciplina
            : null,
          impressao.IdDisciplina7
            ? impressao.IdDisciplina7.nomeDisciplina
            : null,
          impressao.IdDisciplina8
            ? impressao.IdDisciplina8.nomeDisciplina
            : null,
          impressao.IdDisciplina9
            ? impressao.IdDisciplina9.nomeDisciplina
            : null,
          impressao.IdDisciplina10
            ? impressao.IdDisciplina10.nomeDisciplina
            : null,
        ],
      };
    });

    res.render("impressao/index", {
      impressaos: impressaos,
      nomeDisciplinasAtribuidas,
      nomesMateriasAtribuidas,
      posiDisciplinaAtual,
      msgOK,
      msgNOK,
      filePath,
...
exports.enviaIDsProfessores = async (req, res, next) => {
  const mesSelecionado = req.body.mes;
  const anoSelecionado = req.body.ano;
  let idAtual;
  
// 1. Buscar os nomes dos professores associados aos IDs de impressão
Impressao.findAll({
  attributes: ['id', 'professorId'] // Obtém IDs de impressão e IDs de professores associados
}).then(impressoes => {
  const professorIds = impressoes.map(impressao => impressao.professorId);
  // Consulta à tabela de professores para obter os nomes dos professores
  Professor.findAll({
      where: { id: professorIds },
      attributes: ['id', 'nome']
  }).then(professores => {
      // Mapear IDs de professores para nomes de professores
      const idNomeMap = new Map();
      professores.forEach(professor => {
          idNomeMap.set(professor.id, professor.nome);
      });

      // Ordenar os nomes dos professores em ordem ascendente
      const nomesOrdenados = professores.map(professor => professor.nome).sort((a, b) => a.localeCompare(b));

      // Mapear nomes de professores ordenados para IDs de professores
      const idOrdenadoMap = new Map();
      nomesOrdenados.forEach(nome => {
          const id = [...idNomeMap].find(([key, value]) => value === nome)[0];
          idOrdenadoMap.set(nome, id);
      });

      // Mapear IDs de impressão para nomes de professores
      const idImpressaoMap = new Map();
      impressoes.forEach(impressao => {
          const nomeProfessor = idNomeMap.get(impressao.professorId);
          idImpressaoMap.set(impressao.id, nomeProfessor);
      });

      // Ordenar os IDs de impressão com base no novo ordenamento dos nomes dos professores
      const idsOrdenadosPorNome = nomesOrdenados.map(nome => {
          const idsImpressao = [...idImpressaoMap].filter(([key, value]) => value === nome).map(([key]) => key);
          return idsImpressao;
      }).flat();

      console.log(idsOrdenadosPorNome); // IDs de impressão ordenados por nome de professor

      for (i = 0; i < idsOrdenadosPorNome.length; i++) {
        idAtual = idsOrdenadosPorNome[i]
        renderGeraRegistroTodos(idAtual, mesSelecionado, anoSelecionado, res);
...
res.redirect("/impressaos/?msgFilePath=" + 'Downloads');

}
function renderGeraRegistroTodos (id, mesSelecionado, anoSelecionado, res)  {
  ...
  let horarioFormatado = 0;
    ...
    Impressao.findOne({
      ...
      Disciplina.findByPk(impressao.idDisciplina1).then((disciplina) => {
        ...
        ha = ha / 2;
        totalAulas =
          impressao.aulasDisc1 +
          impressao.aulasDisc2 +
          impressao.aulasDisc3 +
          impressao.aulasDisc4 +
          impressao.aulasDisc5 +
          impressao.aulasDisc6 +
          impressao.aulasDisc7 +
          impressao.aulasDisc8 +
          impressao.aulasDisc9 +
          impressao.aulasDisc10;
        hae = impressao.hae1 + impressao.hae2 + impressao.hae3 + impressao.hae4;
        haec =
          impressao.haec1 + impressao.haec2 + impressao.haec3 + impressao.haec4;
        Curso.findAll({}).then((cursos) => {
            ...
            for (var i = 1; i <= totalDeDisciplinas; i++) {
              if (curso.id == idCursoDisciplinas[i]) {
                if (i == 1) {
                  ...
                  if (
                    verificaNomeCursoRepetido !=
                    nomeCursosAtribuidos[auxPosVazia - 1]
                  ) {
          ...
          Feriado.findAll({
            ...
            for (
              ...
              dayOfWeek[day] = currentDate.toLocaleDateString("pt-br", {
                weekday: "short",
              });
              diasMes[day] = day;
              qtdDiasMes += 1;
              ...
              if (
                ...
                dayOfWeek[day] == impressao.diaSemanaHae1 ||
                dayOfWeek[day] == impressao.diaSemanaHae2 ||
                dayOfWeek[day] == impressao.diaSemanaHae3 ||
                dayOfWeek[day] == impressao.diaSemanaHae4 ||
                dayOfWeek[day] == impressao.diaSemanaHaec1 ||
                dayOfWeek[day] == impressao.diaSemanaHaec2 ||
                dayOfWeek[day] == impressao.diaSemanaHaec3 ||
                ...
                //Preencher hae e haec
                  ...
                  if (verificaPeriodoHae < 12) {
                    horarioFormatado = HoraParaImpressao(
                      impressao.horarioHae1.split(":")
                    );
                    inicioManha[day] = horarioFormatado;
                    haeManha[day] = impressao.hae1;
                  } else if (
                    verificaPeriodoHae >= 12 &&
                    verificaPeriodoHae < 18
                  ) {
                    horarioFormatado = HoraParaImpressao(
                      impressao.horarioHae1.split(":")
                    );
                ...
                if (dayOfWeek[day] == impressao.diaSemanaHaec1) {
                  ...
                  if (verificaPeriodoHaec < 12) {
                    horarioFormatado = HoraParaImpressao(
                      impressao.horarioHaec1.split(":")
                    );
                    inicioManha[day] = horarioFormatado;
                    haecManha[day] = impressao.haec1;
                  } else if (
                    verificaPeriodoHaec >= 12 &&
                    verificaPeriodoHaec < 18
                  ) {
                    horarioFormatado = HoraParaImpressao(
                      impressao.horarioHaec1.split(":")
                    );
            ...
            doc.text(
              `FATEC MOGI MIRIM  -  REGISTRO DE PRESENÇA DE DOCENTE - ${mesSelecionado}/ ${ano}`,
              80,
              5
            );
            ...
            doc.text(
              " " + impressao.professor.numeroRegistro,
              doc.getTextWidth("Professor - Reg.: ") + 10,
              12
            );

            const posAfterNumeroRegistro =
              doc.getTextWidth(
                `Professor - Reg.: ${impressao.professor.numeroRegistro}`
              ) + 20;
            doc.setFont("times", "bold");
            doc.text(" Nome: ", posAfterNumeroRegistro, 12);
            doc.setFont("times", "normal");
            doc.text(
              impressao.professor.nome,
              posAfterNumeroRegistro + doc.getTextWidth(" Nome: "),
              12
            );

            const posAfterNome =
              posAfterNumeroRegistro +
              doc.getTextWidth(` Nome: ${impressao.professor.nome}`) +
              50;
            doc.setFont("times", "bold");
            doc.text(" Categoria: ", posAfterNome, 12);
            doc.setFont("times", "normal");
            doc.text(
              " " + impressao.professor.categoria,
              posAfterNome + doc.getTextWidth(" Categoria: "),
              12
            );

            const posAfterCategoria =
              posAfterNome +
              doc.getTextWidth(` Categoria: ${impressao.professor.categoria}`) +
              50;
            doc.setFont("times", "bold");
            ...
            const posAfterCurso =
              posAfterCategoria + doc.getTextWidth("Curso: ");
            let offsetpos = 0;
            ...
            const posAfterHA =
              posAfterAulas + doc.getTextWidth(` H.A: ${ha}`) + 40;
            doc.setFont("times", "bold");
            ...
            const posAfterHAE =
              posAfterHA + doc.getTextWidth(` H.A.E: ${hae}`) + 40;
            doc.setFont("times", "bold");
            doc.text(" H.A.E.C:", posAfterHAE, 22);
            doc.setFont("times", "normal");
            doc.text(
              `${haec}`,
              posAfterHAE + doc.getTextWidth(" H.A.E.C: "),
              22
            );

            const posAfterHAEC =
              posAfterHAE + doc.getTextWidth(` Jornada:`) + 40;
            doc.setFont("times", "bold");
            ...
            const startY = 24; // Posição Y inicial abaixo do cabeçalho
             ...
             // Conteúdo da célula mesclada em "Observações"
              const observacoesContent = `
              Controle Interno


              H.Aula/Mês____  H.Ativ/Mês____

              HAEO/Mês:_____  HAEC/Mês:_____
              ...
              Reposições/Mês:______

              Substituições/Mês:____
            ...
            for (let day = 1; day <= qtdDiasMes; day++) {
              ...
              if (day === 13) {
                // Adicionar célula mesclada para as observações do dia 13 ao dia 28
                rowData.push({
                  content: observacoesContent,
                  rowSpan: 16,
                  styles: { valign: 'middle', halign: 'left' }
              });
            } else if (day > 13 && day <= 28) {
                // Preencher células de observações mescladas com vazio
                rowData.push('');
            } else {
                // Para os dias fora do intervalo de 13 a 28, adicionar célula normal
                rowData.push('');
            }
            ...
            // Formatando o nome do arquivo
             const nomeArquivo = `registro_de_ponto_${mesSelecionado}_${impressao.professor.nome.replace(
               / /g,
               "_"
             )}.pdf`;
    ...
    Disciplina.findByPk(impressao.idDisciplina1).then((disciplina) => {
      ...
      Curso.findAll({}).then((cursos) => {
        ...
        Feriado.findAll({
          ...
          // res.redirect("/impressaos/?msgFilePath=" + filePath);

           // Gerar o nome do arquivo
           const fileName = `registro_de_ponto_${mesSelecionado}_${impressao.professor.nome.replace(/ /g, '_')}.pdf`;
...
exports.renderNovo = (req, res, next) => {
  ...
  Professor.findAll({
    order: [["nome", "ASC"]],
    attributes: ["id", "nome", "numeroRegistro"],
  }).then((professors) => {
    Disciplina.findAll({
  ...
  Disciplina.findOne({
    ...
    Impressao.findOne({
      ...
      if (impressao == undefined) {
        //if(impressao == undefined && (horarioOk)) se for checar horário
        Impressao.create({
          professorId: professorId,
